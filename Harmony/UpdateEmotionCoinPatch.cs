...
namespace UtilLoader21341.Harmony
{
    ...
    public class UpdateEmotionCoinPatch
    {
        ...
        private static Predicate<BattleUnitModel> Match => x =>
            ...
            ModParameters.StageOptions.Any(y =>
                y.PackageId == Singleton<StageController>.Instance.GetStageModel().ClassInfo.id.packageId &&
                y.StageId == Singleton<StageController>.Instance.GetStageModel().ClassInfo.id.id &&
                y.BannedEmotionLevel) ||
            BattleObjectManager.instance.GetAliveList(x.faction).Any(a =>
                ModParameters.PassiveOptions.Any(b => a.passiveDetail.PassiveList.Exists(c =>
                    b.PackageId == c.id.packageId && b.PassiveId == c.id.id && b.BannedEgoAndEmotionCards)));
